...
void Model::loadH5File(std::string const& fileName)
{
	...
	for (unsigned int i = 0; i < uNumLayers; i++)
	{
		...
		// Read the bias
		dlib::matrix<float, 0, 1> bias;
		hdf5Reader->readDataset(bias_path, bias);

		// Read the kernel
		dlib::matrix<float, 0, 0, dlib::default_memory_manager, dlib::row_major_layout> kernel;
	...
	if (hdf5Reader->checkExistence("/wavelengths"))
	{
		...
		dlib::matrix<float, 0, 1> outputGrid;
...
ScalerCSPtr Model::loadScaler(HDF5InterfaceCSPtr const& hdf5Reader, std::string const& scalerPath)
{
	...
	if (type == "MinMaxScaler")
	{
		...
		// Read the scale
		dlib::matrix<double, 0, 1> scale;
		hdf5Reader->readDataset(scalePath, scale);

		// Read the min
		dlib::matrix<double, 0, 1> min;
...
void Model::predict(dlib::matrix<double, 0, 1> const& input, dlib::matrix<double, 0, 1>& output) const
{
	...
	for (unsigned int i = 0; i < m_vLayers.size(); i++)
	{
		...
		if (i == 0)
		{
			...
			{
				dlib::matrix<double, 0, 1>& outputTmp = workspace.m_vIntermediateResults[0];
				// Check if the input has to be scaled
				if (m_pInputScaler)
				{
		...
		{
			dlib::matrix<double, 0, 1>& inputTmp = workspace.m_vIntermediateResults[i-1];
		...
		{
			dlib::matrix<double, 0, 1>& inputTmp = workspace.m_vIntermediateResults[i-1];
			dlib::matrix<double, 0, 1>& outputTmp = workspace.m_vIntermediateResults[i];
...
void Model::predict(dlib::matrix<double, 0, 1> const& input, dlib::matrix<double, 0, 1>& output, dlib::matrix<double>& der) const
{
	unsigned int uThreadNum = 0;
	#ifdef _OPENMP
	uThreadNum = omp_get_thread_num();
	...
	for (unsigned int i = 0; i < m_vLayers.size(); i++)
	{
		...
		if (i == 0)
		{
			...
			{
				dlib::matrix<double, 0, 1>& outputTmp = workspace.m_vIntermediateResults[0];
				dlib::matrix<double>& derTmp = workspace.m_vIntermediateDerivatives[0];
				if (m_pInputScaler)
				{
					// Scale the inputs
		...
		{
			dlib::matrix<double, 0, 1>& inputTmp = workspace.m_vIntermediateResults[i-1];
			dlib::matrix<double>& der_inputTmp = workspace.m_vIntermediateDerivatives[i-1];
		...
		else
		{
			...
			dlib::matrix<double, 0, 1>& outputTmp = workspace.m_vIntermediateResults[i];
			dlib::matrix<double>& der_inputTmp = workspace.m_vIntermediateDerivatives[i-1];
			dlib::matrix<double>& der_outputTmp = workspace.m_vIntermediateDerivatives[i];
	...
	if (m_pInputScaler)
	{
		...
		{
			// TODO: Check if this works - not that a transpose is needed
			dlib::set_rowm(der, i) = dlib::rowm(der, i) * m_pInputScaler->getScaleDerivative()(i);
		}
	}
	...
	if (m_pOutputScaler)
	{
		...
		for (unsigned int i = 0; i < der.nr(); i++)
		{
			dlib::set_rowm(der, i) = dlib::pointwise_multiply(dlib::rowm(der, i), m_pOutputScaler->getScaleInverseDerivative());
		}
	}
}
...
Model::Workspace::Workspace(std::vector<unsigned int> const& vuDimensions)
{
	...
	{
		m_vIntermediateResults.emplace_back(dlib::matrix<double, 0, 1>(vuDimensions[i]));
	}
	for (unsigned int i = 1; i < vuDimensions.size(); i++)
	{
		m_vIntermediateDerivatives.emplace_back(dlib::matrix<double>(vuDimensions[0], vuDimensions[i]));
	}
}
