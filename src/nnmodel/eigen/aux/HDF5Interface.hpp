...
public:
	...
	template< typename T>
	void readDataset(std::string const& path, Eigen::Matrix<T, Eigen::Dynamic, 1>& data) const;
	template <typename T>
	void readDataset(std::string const& path, Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& data) const;
